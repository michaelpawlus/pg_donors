...
qplot(age, log_tg,data=pg,color=const,size=gillett,shape=married,main="Constituency/Married PG Donors")
qplot(age, lub_yrs,data=pg,color=cae,size=gillett,shape=married,main="CAE/Married PG Donors")
qplot(age, log_tg,data=pg,color=cae,size=gillett,shape=married,main="CAE/Married PG Donors")
...
# K-Nearest Neighbor
knnFit <- train(gillett ~ rating  + age + lub_yrs + child_count + log_tg + const + cae, 
                method = "knn", 
                data = training,
                tuneLength = 5,  
...
confusionMatrix(knn.pred,testing$gillett)
